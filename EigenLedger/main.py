...
import warnings
import logging
from EigenLedger.modules.empyrical import (
    cagr,
    cum_returns,
...
class Engine:
    ...
    def fetch_benchmark_data(self):
        ...
        if not self.data.empty and all(b in self.data.columns for b in self.benchmark):
            ...
            try:
                self.benchmark_data = yf.download(self.benchmark, start=self.start_date, auto_adjust=False)["Adj Close"]
...
def get_returns(stocks, wts, start_date, end_date=TODAY):
    ...
    for stock in stocks:
        try:
            asset = yf.download(stock, start=start_date, end=end_date, progress=False, auto_adjust=False, actions=True)["Adj Close"]
            if not asset.empty:
                available_stocks.append(stock)
            else:
                logging.warning(f"Debug: {stock} returned empty data.")
    ...
    if missing_stocks:
        logging.warning("Missing stock(s): %s", missing_stocks)
    ...
    if len(available_stocks) > 1:
        data = yf.download(available_stocks, start=start_date, end=end_date, progress=False, auto_adjust=False, actions=True)
        assets = data["Adj Close"]
        dividends = data["Dividends"] if "Dividends" in data.columns else pd.DataFrame(0, index=assets.index, columns=assets.columns)
        
        assets = assets.filter(available_stocks)
        dividends = dividends.filter(available_stocks)
        assets = assets.bfill().dropna() # Backfill to handle missing start data, then drop remaining NaNs
        # Align dividends to assets index
        dividends = dividends.reindex(assets.index).fillna(0)
        ...
        # Dividends per share * Shares held (initial_alloc)
        # We assume constant shares for this static backtest
        portfolio_dividends = (dividends * initial_alloc).sum(axis=1)
        
        logging.info("Returning returns with multiple stocks.")
        return returns, portfolio_dividends
    
    elif len(available_stocks) == 1:
        data = yf.download(available_stocks[0], start=start_date, end=end_date, progress=False, auto_adjust=False, actions=True)
        df = pd.DataFrame(data["Adj Close"])
        divs = pd.DataFrame(data["Dividends"]) if "Dividends" in data.columns else pd.DataFrame(0, index=df.index, columns=df.columns)
        
        returns = df.pct_change().dropna()
        # For single stock, initial alloc is 1/price
        initial_alloc = 1.0 / df.iloc[0, 0]
        portfolio_dividends = divs.iloc[:, 0] * initial_alloc
        portfolio_dividends = portfolio_dividends.reindex(returns.index).fillna(0)
...
def portfolio_analysis(my_portfolio, rf=0.0, sigma_value=1, confidence_value=0.95, report=False, filename="report.pdf"):
    ...
    if isinstance(my_portfolio.rebalance, pd.DataFrame):
        ...
        for i in range(len(dates) - 1):
            ...
            add_returns, add_dividends = get_returns(
            ...
            # TODO: Handle rebalancing dividends appending (simplified for now)
        ...
        if not my_portfolio.data.empty:
            ...
            portfolio_dividends = pd.Series(0, index=returns.index) # Placeholder for custom data without dividends
        else:
            returns, portfolio_dividends = get_returns(
                my_portfolio.portfolio,
                my_portfolio.weights,
    ...
    # Dividend Metrics
    total_dividend_income = portfolio_dividends.sum()
    # Initial portfolio value is 1.0 (implied by initial_alloc calculation logic)
    dividend_yield = total_dividend_income / 1.0 
    dividend_yield_pct = str(round(dividend_yield * 100, 2)) + "%"
    
    # ... (Risk manager logic remains same) ...
    ...
    try:
        if list(my_portfolio.risk_manager.keys())[0] == "Stop Loss":
            ...
            try:
                ...
                portfolio_dividends = portfolio_dividends[: my_portfolio.end_date]
    ...
    # Recalculate dividend metrics if date range changed
    total_dividend_income = portfolio_dividends.sum()
    dividend_yield = total_dividend_income / 1.0 
    dividend_yield_pct = str(round(dividend_yield * 100, 2)) + "%"
    ...
    if not my_portfolio.benchmark_data.empty:
        ...
        # Get benchmark returns from benchmark data
        benchmark = get_returns_from_benchmark_data(my_portfolio.benchmark_data, None, my_portfolio.benchmark)
        ...
        # Benchmark returns usually don't need dividends explicitly for comparison unless Total Return
        benchmark, _ = get_returns(
    ...
    # Display dataframe (works in Jupyter and terminal)
    try:
