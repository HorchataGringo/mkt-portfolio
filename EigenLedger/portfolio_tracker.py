    ...
    for index, row in portfolio_df.iterrows():
        ...
        try:
            ...
            if purchase_price == 0 or pd.isna(purchase_price):
                import logging
                logging.error(f"Invalid purchase price (${purchase_price}) for {ticker} on {purchase_date}. Skipping position.")
                continue
        except Exception as e:
            import logging
            logging.error(f"Could not find price for {ticker} on {purchase_date}: {e}. Skipping position.")
            continue
...
def main():
    ...
    df = load_portfolio(str(dad_tickers_path))
...
"""
    ...
    if ENABLE_CLOUD:
        # Save report to CSV
        report_path = base_dir / "portfolio_report.csv"
        metrics.to_csv(str(report_path), index=False)

        # Upload to Drive
        if drive_client:
            drive_client.upload_file(str(report_path), folder_id=FOLDER_ID)
